...
任何一个函数都可以被new，new了之后，就成了构造方法。（有歧义，我的理解是只有构造函数才可以new）
...
所有的**函数**（不包括数组、对象），都有一个`prototype`属性，属性值是一个**普通的对象**。`prototype`的含义是**显式原型**。（实例没有这个属性）(确实，prototype,应该是属于函数的，对象是不具有prototype属性的，但是提到函数也是也是一个对象，所以。。。。)
