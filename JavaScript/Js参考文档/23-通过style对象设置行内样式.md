...
上图显示，因为border属性不是行内样式，所以无法通过style对象获取。此处的border被设置到了内嵌样式中，所以在这里

~~~javascript
dom1.style.border获取不到但是应该可以通过
dom1.className 来获取，但是还是有问题
~~~
...
方式二最大的优点是：可以给属性传递参数。（此处有问题，因为·方式一，也是可以给属性传递参数的，应该改为可以给style这个对象添加属性值，但是貌似所有的对象都可以这样添加属性，方式二的话，适用于遍历对象的时候，利用[key]动态获取对象的属性值）
...
备注：我们通过style属性设置的样式都是**行内样式**，而行内样式有较高的优先级。但是如果在样式中的其他地方写了`!important`，则此时`!important`会有更高的优先级。（所以要慎用!import,因为一旦使用了，就不好更改这个样式了）
