...
既然方式二、方式三获取的是标签数组，那么习惯性是**先遍历之后再使用**（这是一个伪数组）。
...
// 这也提供了一个下标索引遍历伪数组的方法，当然，也可以采用ES5新增的Array.from(伪数组)来重新生成一个新的数组
// 伪数组的定义参考，这里就不写了
// 这里同时也可以采用兼容模式也可以生成一个新数组出来
//[].slice.apply(伪数组,0),这样也可以遍历出来，这里又涉及到了call与apply的区别了，前者以变量形式，后者以数组形式传参数。
// slice如果没有传递参数，默认截取的是全部的值
// slice(startIndex,endIndex);从数组中截取指定的元素，返回截取的元素保存的数组
[].slick.call($div) // $div为jquery中获取的伪数组。
```
