...
> 补充，字符串属于基本数据类型，但是在字符串中有时候可以使用某些属性和方法

~~~javascript
var Str = 'aaaa';
console.log(str.lenght)
console.log(str.indexOf())
// 是因为javascript的底层对字符串的基本数据类型做了临时包装，将基本数据类型字符串转成了包装类型字符串。也就是包装成了一个对象，也就是有了属性
var str1 = new String('aaaaa')// 采用这种方式可以手动转成对象，具体参考包装类型。
~~~
...
在底层，字符串是以**字符数组**的形式保存的。代码举例：

```javascript
	var str = "smyhvae";
	console.log(str.length); // 获取字符串的长度
	console.log(str[2]); // 获取字符串中的第2个字符,但是用的不多
```
...
`charAt`：返回字符串指定位置的字符。不会修改原字符串。（准确理解应该以字符串形式返回一个指定位置的字符）
...
解释：字符串中第一个字符的下标是 0。如果参数 index 不在 [0, string.length) 之间，该方法将返回一个空字符串。（补充：此处是否应该为一个字符，而不是一个空字符串呢？经测试理解错误）
...
console.log(typeof str.charAt(9));//string
var st = str.charAt(9);//空串
...
上面这个例子一般不用。一般打印数组和json的时候用索引，打印String不建议用索引。应该用遍历要好一些，遍历用索引。无论数组还是字符串都可以用索引
...
// 这里突然有一个思路，就是判断字符串中的字符是否唯一，能够可以利用indexOf和lastIndexOf的值相等来进行处理呢？理论上貌似可以的
```
...
```javascript
    ...
    var str1 = 'abcdedafdseas';
    // indeOf返回的是字符的索引，第二个参数不是索引，而是第几个字符
    console.log(str1.indexOf('a',1));// 6
    console.log(str1.indexOf('a',6));// 6
    console.log(str1.indexOf('a',7));// 11
```
...
- (1, -1) 表示从第一个截取到倒数第一个。
  ...
  ~~~javascript
  var str = 'abcedefg';
  console.log(str.slice(1,-1));bcedef
  ~~~
...
- （补充：总结，既然跟slice()方法一样，还不如slice()方法更加给力，这个API比较鸡肋。）
...
备注：ECMAscript 没有对 `substr()` 方法进行标准化，因此不建议使用它。（补充：兼容性不好么？）
...
```javascript
    ...
    var str = '人间天堂|hello World!|hehehe'
    console.log(str);//人间天堂|hello World!|hehehe
    console.log(str.split());//[ '人间天堂|hello World!|hehehe' ]
    ...
    //   ]
    console.log(str.split('|'));//[ '人间天堂', 'hello World!', 'hehehe' ]
    console.log(str.split('h'));//[ '人间天堂|', 'ello World!|', 'e', 'e', 'e' ]
```
